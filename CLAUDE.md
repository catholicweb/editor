...
- **`store.js`** — the single reactive state object (`state`). Owns login/bootstrap, session persistence in `localStorage`, opening a tab (applies schema defaults, sets `baselineText`), dirty tracking (**serialized draft vs `baselineText`**), debounced autosave (10s) plus a `visibilitychange` flush, explicit save (`saveCurrent` → PUT of the whole `config.json`), media upload, and dynamic theming from `config.json` (accent color, Google Fonts). ⚠️ Theme values are sanitized (`validHexColor`, `sanitizeFontName`) before reaching CSS/DOM — a malformed config value must never inject CSS/HTML.
