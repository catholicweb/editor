...
- **`codec.js`** — flat-filename encode/validate (URL-safe charset, `/`→`-`, `ALLOWED_EXT`, 255-char limit, no leading `-`). Used only to **name new media uploads** (the R2 object key) — media field values are absolute URLs and are never decoded here. ⚠️ The filename rules **MUST stay byte-for-byte compatible** with `config-api/src/index.js` (which still validates the PUT `/sites/:slug/:token` path). See the header warning in the file and `../CLAUDE.md`.
- **`schema.js`** — normalizes the parsed `pages.yml` into a resolved field tree. `resolveFieldDef` implements `component:` inheritance (a field/block-variant inherits `type`/`options`/`fields`/`blocks` from the referenced component; its own explicit keys win). Also owns default-value logic (`defaultForField`/`applyDefaults`/`defaultBlockItem`), the polymorphic-block discriminator constant **`BLOCK_TYPE_KEY = 'type'`** (the only place that convention is encoded — change it if the static-site generator expects a different key), and collapsible-summary interpolation (`{fields.title}` patterns via `renderSummary`). `normalizeSchema` also extracts `components.event` fields into `schema.eventFields` for the calendar editor.
- **`content-index.js`** — builds the editable-entries list (`buildFileIndex`, now tab-only) and the media helpers (`mediaPrefix`, `listMediaFiles`, `relPathForNewMedia`). Media field values and the picker list are plain **absolute URLs** (e.g. `https://data.parroquia.app/<slug>/media-x.webp`) returned by the config-api — no `/media/` prefix, no token decoding. `relPathForNewMedia` only names new uploads.
...
- **`store.js`** — the single reactive state object (`state`). Owns login/bootstrap, session persistence in `localStorage`, opening a tab (applies schema defaults, sets `baselineText`), dirty tracking (**serialized draft vs `baselineText`**), debounced autosave (60s) plus a `visibilitychange` flush, explicit save (`saveCurrent` → PUT of the whole `config.json`), media upload, and dynamic theming from `config.json` (accent color, Google Fonts). **Multisession:** one editor token is valid for every slug its email is granted on (`state.email`/`state.slugs`, refreshed from `/whoami`); `switchSlug` flushes pending edits and re-runs the per-slug bootstrap with the same token (schema is shared, kept via `fetchSchema:false`), and AdminView renders a site `<select>` when `state.slugs.length > 1`. ⚠️ Theme values are sanitized (`validHexColor`, `sanitizeFontName`) before reaching CSS/DOM — a malformed config value must never inject CSS/HTML.
...
- **`reference`** fields label each option with the referenced item's `name`/`title` (from within the loaded config), not a document title fetched from disk — avoiding per-collection downloads.
