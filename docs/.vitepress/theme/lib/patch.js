...
// the sibling file; a divergence silently corrupts concurrent saves (uuid
...
//   non-empty string `uuid`. Keyed arrays are diffed/addressed by that stable
//   uuid, so per-field edits survive concurrent saves (last-edit-wins) and a
//   concurrent removal never resurrects an item.
//   ANY OTHER array (scalars, or objects lacking a uuid) is "keyless" and is
//   replaced wholesale on any change.
//
// The schema's hidden `uuid` default (see schema.js UUID_KEY / injectUuid)
...
//
// UUID_KEY here MUST equal UUID_KEY in editor docs/.vitepress/theme/lib/schema.js.
//
...
//                                                last segment is `{ uuid }`) remove
//                                                that keyed list item; no-op if absent
//   - listAdd    { op:'listAdd',    path, uuid, index, value }  insert a new keyed item
//   - listReorder{ op:'listReorder', path, uuids }              set the list's uuid order
//
...
// kept well under 5 ms on ~100 KB docs (a WeakMap uuid→index cache makes
// repeated {uuid} segments O(1) rather than O(n)).
...
function isUuidSegment(seg) {
  return isPlainObject(seg) && typeof seg[UUID_KEY] === 'string';
}
...
// `uuid`. Empty arrays are vacuously keyed. Anything else is keyless.
export function isKeyedArray(arr) {
  if (!Array.isArray(arr)) return false;
  for (const item of arr) {
    if (!isPlainObject(item) || typeof item[UUID_KEY] !== 'string' || item[UUID_KEY].length === 0) {
      return false;
    }
  }
  return true;
...
function diffKeyedArray(base, cur, path, ops) {
  ...
  for (const item of base) {
    if (isPlainObject(item) && typeof item[UUID_KEY] === 'string') bMap.set(item[UUID_KEY], item);
  }
  const baseOrder = base
    .map((item) => (isPlainObject(item) ? item[UUID_KEY] : undefined))
    .filter((u) => typeof u === 'string');

  for (let i = 0; i < cur.length; i++) {
    const item = cur[i];
    let uuid = isPlainObject(item) && typeof item[UUID_KEY] === 'string' ? item[UUID_KEY] : '';
    const bItem = bMap.get(uuid);
    if (bItem !== undefined) {
      // existing item -> recurse per-field at {...path, {uuid}}
      diffNode(bItem, item, path.concat({ [UUID_KEY]: uuid }), ops);
      bMap.delete(uuid);
    } else {
      ...
      const value = isPlainObject(item) ? { ...item } : { [UUID_KEY]: uuid, value: item };
      if (!value[UUID_KEY]) value[UUID_KEY] = uuid;
      ops.push({ op: 'listAdd', path, uuid, index: i, value: deepClone(value) });
    }
  }
  // Remaining base items: removed.
  for (const uuid of bMap.keys()) {
    ops.push({ op: 'remove', path: path.concat({ [UUID_KEY]: uuid }) });
  }
  ...
  for (const item of cur) {
    const u = isPlainObject(item) ? item[UUID_KEY] : undefined;
  ...
  const expected = [];
  const inCur = new Set(curUUIDs);
  for (const u of baseOrder) if (inCur.has(u)) expected.push(u); // survivors in base order
  for (const u of curUUIDs) if (!expected.includes(u)) expected.push(u); // new uuids in cur order
  if (curUUIDs.length > 1 && curUUIDs.join('|') !== expected.join('|')) {
    ops.push({ op: 'listReorder', path, uuids: curUUIDs.slice() });
  }
}
...
function applyOp(root, op) {
  ...
  switch (op.op) {
    ...
    case 'listAdd':
      return applyListAdd(root, op.path, op.uuid, op.index, op.value);
    case 'listReorder':
      return applyListReorder(root, op.path, op.uuids);
    default:
      return false;
  }
}
...
// doc as it goes). `{ uuid }` segments are always resolved strictly: if the
...
function resolveToParent(root, path, create) {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (isUuidSegment(seg)) {
      ...
      const idx = indexByUuid(node, seg[UUID_KEY]);
  ...
  if (isUuidSegment(last)) {
    if (!Array.isArray(node)) return null;
    return { node, lastUuid: last[UUID_KEY] };
  }
  if (!isPlainObject(node)) return null;
  return { node, lastKey: last };
}

// uuid -> index cache keyed by array identity, so repeated {uuid} segments and
// list ops are O(1). Invalidated whenever an array is structurally mutated.
const uuidIndexCache = new WeakMap();
function indexByUuid(arr, uuid) {
  let map = uuidIndexCache.get(arr);
  if (!map) {
    map = new Map();
    for (let i = 0; i < arr.length; i++) {
      const u = arr[i] && arr[i][UUID_KEY];
      if (typeof u === 'string') map.set(u, i);
    }
    uuidIndexCache.set(arr, map);
  }
...
function applySet(root, path, value) {
  ...
  if (p.lastUuid !== undefined) {
    const idx = indexByUuid(p.node, p.lastUuid);
...
function applyListReorder(root, path, uuids) {
  ...
  for (const el of arr) {
    if (isPlainObject(el) && typeof el[UUID_KEY] === 'string') byUuid.set(el[UUID_KEY], el);
  }
  const rebuilt = [];
  ...
  for (const el of arr) {
    if (isPlainObject(el) && typeof el[UUID_KEY] === 'string' && byUuid.has(el[UUID_KEY])) {
      rebuilt.push(el);
      byUuid.delete(el[UUID_KEY]);
    }
  }
  arr.length = 0;
